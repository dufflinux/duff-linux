...
README.md: README.md.in build/mkiso.sh build/mklive.sh build/mkrootfs.sh build/mkplatformfs.sh build/mkimage.sh build/mknet.sh
	...
	for script in mkiso mklive mkrootfs mkplatformfs mkimage mknet; do \
		printf '### %s.sh\n\n```\n' "$${script}" >> README.md ; \
		"./build/$${script}.sh" -h 2>/dev/null >> README.md ; \
		printf '```\n\n' >> README.md ; \
	done

build/mkiso.sh: build/mklive.sh
...
void-live-%.iso: build/mkiso.sh
	@[ -n "${CI}" ] && printf "::group::\x1b[32mBuilding $@...\x1b[0m\n" || true
	$(if $(findstring aarch64,$*), \
		$(SUDO) ./build/mkiso.sh -r $(REPOSITORY) -t $* -- -P "$(LIVE_PLATFORMS)", \
		$(SUDO) ./build/mkiso.sh -r $(REPOSITORY) -t $*)
...
void-%-ROOTFS-$(DATECODE).tar.xz: build/mkrootfs.sh
	@[ -n "${CI}" ] && printf "::group::\x1b[32mBuilding $@...\x1b[0m\n" || true
	$(SUDO) ./build/mkrootfs.sh $(XBPS_REPOSITORY) -x $(COMPRESSOR_THREADS) -o $@ $*
	@[ -n "${CI}" ] && printf '::endgroup::\n' || true
...
.SECONDEXPANSION:
void-%-PLATFORMFS-$(DATECODE).tar.xz: void-$$(shell ./build/lib.sh platform2arch %)-ROOTFS-$(DATECODE).tar.xz build/mkplatformfs.sh
	@[ -n "${CI}" ] && printf "::group::\x1b[32mBuilding $@...\x1b[0m\n" || true
	$(SUDO) ./build/mkplatformfs.sh $(XBPS_REPOSITORY) -x $(COMPRESSOR_THREADS) -o $@ $* void-$(shell ./build/lib.sh platform2arch $*)-ROOTFS-$(DATECODE).tar.xz
	@[ -n "${CI}" ] && printf '::endgroup::\n' || true
...
void-%-$(DATECODE).img.xz: void-%-PLATFORMFS-$(DATECODE).tar.xz build/mkimage.sh
	@[ -n "${CI}" ] && printf "::group::\x1b[32mBuilding $@...\x1b[0m\n" || true
	$(SUDO) ./build/mkimage.sh -x $(COMPRESSOR_THREADS) -o $(basename $@) void-$*-PLATFORMFS-$(DATECODE).tar.xz
	@[ -n "${CI}" ] && printf '::endgroup::\n' || true

# Some of the images MUST be compressed with gzip rather than xz, this
# rule services those images.
void-%-$(DATECODE).tar.gz: void-%-PLATFORMFS-$(DATECODE).tar.xz build/mkimage.sh
	@[ -n "${CI}" ] && printf "::group::\x1b[32mBuilding $@...\x1b[0m\n" || true
	$(SUDO) ./build/mkimage.sh -x $(COMPRESSOR_THREADS) void-$*-PLATFORMFS-$(DATECODE).tar.xz
	@[ -n "${CI}" ] && printf '::endgroup::\n' || true
...
void-%-NETBOOT-$(DATECODE).tar.gz: void-%-ROOTFS-$(DATECODE).tar.xz build/mknet.sh
	@[ -n "${CI}" ] && printf "::group::\x1b[32mBuilding $@...\x1b[0m\n" || true
	$(SUDO) ./build/mknet.sh void-$*-ROOTFS-$(DATECODE).tar.xz
	@[ -n "${CI}" ] && printf '::endgroup::\n' || true
